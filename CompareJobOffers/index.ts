...
const compareJobOffers: AzureFunction = async (context: Context, req: HttpRequest): Promise<void> => {
    ...
    if (!req.body || !req.body.offer1ID || !req.body.offer2ID) {
        context.res = responseFactory("Need to include offer1 and offer2ID in the request body.", 401);
        return;
    }
    ...
    if (offer1.user_id !== user_id || offer2.user_id) {
    ...
    const { resources: offer1Cities }: { resources: City[] } = await jobOffersContainer.items.query(city1Query).fetchAll();
    const { resources: offer2Cities }: { resources: City[] } = await jobOffersContainer.items.query(city2Query).fetchAll();
    ...
    // Comparison
    const offerComparison: JobOfferComparison = {
        offer1ID: offer1.id,
